...
export const Navbar: React.FC<NavbarProps> = ({ username, onLogout }) => {
  ...
  return (
    <nav
      ...
      ${scrolled ? "bg-white/80 backdrop-blur-md shadow-md border-b" : "bg-white"}`}
    >
      {/* LOGO + FLIP ANIMATION */}
      <div className="flex items-center gap-3 group cursor-pointer">
        ...
        <div className="relative w-36 h-6 perspective-500 hidden sm:block">
          <span className="absolute inset-0 flex items-center justify-start text-gray-800 font-bold text-xl transition-all duration-500 group-hover:rotate-y-180 opacity-100 group-hover:opacity-0">
            NXT
          </span>

          <span className="absolute inset-0 flex items-center justify-start text-gray-800 font-bold text-xl rotate-y-180 transition-all duration-500 group-hover:rotate-y-0 opacity-0 group-hover:opacity-100">
            NXT-ABOGADOS
          </span>
        </div>
      </div>
